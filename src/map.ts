...
function vertIt(points:Array<Array<vec3>>, normals:Array<Array<vec3>>):Float32Array
{
...
export class TankMap
{
    ...
    constructor(program:WebGLProgram, color:vec3, extremety:number, width:number, height:number, smoothness:number, tesselation:number)
    {
        ...
        let trueWidth = Math.pow(2, width)+1;
        this.points = Array<Array<vec3>>();
        this.normals = Array<Array<vec3>>();
    ...
    createMap(width:number, height:number, extremety:number, smoothness:number, tesselation:number)
    {

    
    let trueWidth = Math.pow(2, width)+1;
    this.points = Array<Array<vec3>>(trueWidth);
    let ds = getDiamondSquare(width, height/2, extremety);
    //printDS(ds);
    for(let i = 0; i < smoothness; i++)
    {
    ...
    for(let x = 0; x < ds.length; x++)
    {
        this.points[x] = Array<vec3>(trueWidth);
        for(let y = 0; y < ds.length; y++)
        {
    ...
    this.normals = Array<Array<vec3>>(trueWidth);
    for(let x = 0; x < ds.length; x++)
    {
        this.normals[x] = Array<vec3>(trueWidth);
        for(let y = 0; y < ds.length; y++)
        {
            let min = 0;
            let max = ds.length-1;
